...
const Homepage = () => {
  ...
  const fetchAllUrls = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/allUrls`
      );
  ...
  //   -------------- ending of fetching all url  -----------------

  //   -------------- logic for changing between tabs  -----------------
  const handleTabChange = (event, newValue) => {
    setTabValue(newValue);
  };

  //   -------------- logic for the submission for encoding urls  -----------------
  const handleEncodeSubmit = async (event) => {
    event.preventDefault();

    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/encode`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            expirationMinutes: showExpirationDate
              ? expirationDate.trim()
              : undefined,
  ...
  //   -------------- logic for the submission for decoding urls  -----------------
  const handleDecodeSubmit = async (event) => {
    event.preventDefault();

    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/decode`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  //   -------------- logic for the selecting individual items on the table  -----------------
  const handleRowClick = (url) => {
    setSelectedUrl(url);
    setDialogOpen(true);
  };

  //   -------------- logic for the closing of dialog  -----------------
  const handleDialogClose = () => {
    setSelectedUrl(null);
    setDialogOpen(false);
  };

  //   -------------- logic for copying particular encoded url to clipboard  -----------------
  const handleCopyUrl = () => {
    if (selectedUrl) {
      navigator.clipboard.writeText(
        `${process.env.REACT_APP_SERVER_URL}/api/encodedUrl/${selectedUrl.encodedUrl}`
      );
    }
  ...
  //   -------------- logic for deleting particular encoded url  -----------------
  const handelDeleteUrl = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/deleteUrl/${selectedUrl._id}`,
        {
          method: "DELETE",
        }
      );
      ...
      <Container>
        ...
        >
          <Typography variant="h5" sx={{ textAlign: "center" }}>
            {encodedUrlResult && tabValue === 0 ? (
              <ResultContainer>
                {process.env.REACT_APP_SERVER_URL}/{encodedUrlResult}{" "}
              </ResultContainer>
            ) : null}
            {decodedUrlResult && tabValue === 1 ? (
              <ResultContainer>{decodedUrlResult}</ResultContainer>
            ) : null}
          </Typography>
        </Box>
